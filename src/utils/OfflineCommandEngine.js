...
export async function processCommand(text) {
    ...
    try {
      ...
      switch (match.type) {
        ...
        case 'media':
          ...
          break
        case 'help':
          result.response = 'I can open apps, make phone calls, turn on the flashlight, control brightness and volume, set timers, alarms, reminders, search the web, and check the weather. Just say what you need.'
          result.keepListening = true
          break
        case 'routine':
        ...
        default:
          if (rsReady && rs) {
            try {
              window._irisRiveSideEffects = window._irisRiveSideEffects || []
              window._irisRiveCommands = window._irisRiveCommands || []
              window._irisRiveSideEffects.length = 0
              window._irisRiveCommands.length = 0
              
              let reply = await rs.reply('localuser', match.text)
              if (reply && reply !== 'ERR: No Reply Matched') {
                result.response = reply
                if (Array.isArray(window._irisRiveSideEffects) && window._irisRiveSideEffects.length > 0) {
                  result.sideEffects.push(...window._irisRiveSideEffects)
                }
                if (Array.isArray(window._irisRiveCommands) && window._irisRiveCommands.length > 0) {
                  result.commands.push(...window._irisRiveCommands)
                }
                if (reply.toLowerCase().includes('goodbye')) result.keepListening = false
                break
              }
            } catch (e) {
              console.error('[Iris] RiveScript fallback error:', e)
            }
          }
          result.response = 'I\'m not sure what you mean. Try saying "open" followed by an app name, or ask for the weather.'
          result.keepListening = true
          break
      }
      ...
      return {
        success: true,
        response: 'I\'m ready. What would you like me to do?',
        commands: [],
        sideEffects: [],
        keepListening: true
      }
  ...
  return new Promise(resolve => {
    commandQueue = commandQueue.then(() => run().then(resolve)).catch(err => {
      console.error('[Iris] commandQueue error:', err)
      resolve({
        success: true,
        response: 'I\'m ready. What would you like me to do?',
        commands: [],
